...
// ---------------------------------------------------------------------------
// Overspending resolution
// ---------------------------------------------------------------------------

Future<void> _checkOverspendBeforeNext(
  BuildContext context,
  BudgetState currentState,
  BudgetNotifier notifier,
  DateTime nextMonth,
) async {
  final overspent = currentState.groups
      .expand((g) => g.entries)
      .where((e) => e.balance < 0)
      .toList();

  if (overspent.isEmpty) {
    notifier.goToMonth(nextMonth);
    return;
  }
...
class _OverspendSheetState extends State<_OverspendSheet> {
  // true = cover from TBB, false = carry forward
  late final Map<String, bool> _decisions;
  bool _saving = false;

  @override
  void initState() {
    super.initState();
    _decisions = {for (final e in widget.entries) e.categoryId: false};
  }
  ...
  Future<void> _apply() async {
    ...
    try {
      for (final e in widget.entries) {
        if (_decisions[e.categoryId] == true) {
          // Cover from TBB: increase budget to zero out the negative
          await widget.notifier.setBudgeted(
            e.categoryId,
            e.budgeted + e.balance.abs(),
          );
        }
      }
  ...
  Widget build(BuildContext context) {
    final cs  = Theme.of(context).colorScheme;
    final fmt = NumberFormat.currency(symbol: '\$', decimalDigits: 2);
    final monthLabel = DateFormat('MMMM').format(widget.currentMonth);
...
class _OverspendRow extends StatelessWidget {
  final BudgetEntry entry;
  final NumberFormat fmt;
  final bool coveredFromTbb;
  final ValueChanged<bool> onToggle;

  const _OverspendRow({
    required this.entry,
    required this.fmt,
    required this.coveredFromTbb,
    required this.onToggle,
  });
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              _ResolutionChip(
                label: 'Cover from TBB',
                selected: coveredFromTbb,
                onTap: () => onToggle(true),
              ),
              const SizedBox(width: 8),
              _ResolutionChip(
